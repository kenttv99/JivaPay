...
- [/] **Общие Инструменты Качества Кода**:
    - [/] Конфигурация ESLint для TypeScript/React (частично реализовано).
...
- [x] **Разработка Базовых Компонентов**:
    ...
    - [x] Card (базовая реализация)
    - [x] Input (базовая реализация)
    ...
    - [ ] Iconography system
- [x] **Темизация**:
    - [x] Реализация темизации через CSS Custom Properties (базовые переменные и тема admin_app реализованы).
    - [ ] Механизм смены тем (например, светлая/темная), если требуется.
- [x] **Кастомизация**: Базовые компоненты поддерживают настройку через пропсы и переменные.
...
- **Начальная Настройка**:
    ...
    - [x] SPA-архитектура: все внутренние приложения реализованы как SPA (без SSR), что упрощает инфраструктуру и ускоряет разработку.
...
- **Анимации (`animations/`)**:
    - [x] Определение и реализация базовых анимаций (fadeIn, skeleton) для UX.
