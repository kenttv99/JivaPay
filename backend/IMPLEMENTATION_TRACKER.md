...
- [ ] **Конфигурация**:
    - [ ] Обработка переменных окружения (`.env`, `pydantic-settings`) для секретов и базовых подключений.
    ...
    - [ ] Создание скрипта `seed` для настроек по умолчанию.
    - Примечания: Часть настроек перенесена в БД для гибкости управления.
...
- [/] **Сервис Подбора Реквизитов (`services/requisite_selector.py`)**: Логика поиска и выбора подходящих реквизитов для incoming orders.
    - Примечания: Создан файл-заглушка с TODO по логике реализации.
- [/] **Сервис Обработки Заявок (`services/order_processor.py`)**: Оркестрация обработки входящих заявок, подбор реквизитов, создание OrderHistory.
    - Примечания: Создан файл-заглушка с TODO по логике реализации (вкл. 2-фазное обновление статуса).
- [/] **Сервис Обновления Балансов (`services/balance_manager.py`)**: Надежное обновление балансов и запись истории.
    - Примечания: Создан файл-заглушка с TODO по логике реализации.
...
- [ ] **Утилиты Работы с S3 (`utils/s3_client.py`)**: Загрузка/управление файлами в S3.
    - Примечания:
