...
async def get_administrators_statistics_admin_panel(
    ...
    return await user_service.get_administrators_statistics(db, current_admin, availability_filter, page, per_page)
...
async def get_supports_statistics_admin_panel(
    ...
    return await user_service.get_supports_statistics(db, current_admin, status_filter, role_description, page, per_page)
...
async def get_teamleads_statistics_admin_panel(
    ...
    return await user_service.get_teamleads_statistics(db, current_admin, status_filter, page, per_page)
...
async def get_administrator_details_admin_panel(
    ...
    return await user_service.get_administrator_details(db, admin_id_to_view=admin_user_id, current_admin_user=current_admin)
...
async def list_all_users_basic(
    ...
    """
    return await user_service.get_all_users_basic(db_session=db, current_admin_user=current_admin)
...
async def update_administrator_profile_admin_panel(
    ...
    return await user_service.update_administrator_profile(db, admin_id_to_update=admin_user_id, profile_data=profile_data, current_admin_user=current_admin)
...
async def get_user_permissions_admin_panel(
    ...
    can_view_specific_role_perms = await perm_service.check_permission(
        user_id=current_admin.id,
        user_role="admin", # current_admin is always an admin here
        required_permission_template=f"permissions:view:{target_role}"
    )
    can_view_any_role_perms = await perm_service.check_permission(
        user_id=current_admin.id,
        user_role="admin",
        required_permission_template="permissions:view:*"
    )
    is_superuser = await perm_service.check_permission(
        user_id=current_admin.id,
        user_role="admin",
    ...
    return await perm_service.get_user_permissions(user_id=target_user_id, user_role=target_role)
...
async def update_user_permissions_admin_panel(
    ...
    perm_service = PermissionService(db)
    success = await perm_service.update_user_permissions(
